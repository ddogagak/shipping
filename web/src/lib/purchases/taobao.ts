...
export function parseTaobaoWorkbook(buffer:ArrayBuffer):TaobaoOrder[]{
  ...
  for(const r of rows){
    ...
    const q=qty(r["商品数量"]), unitPrice=money(r["商品金额"]);
    current.items.push({product_name:name,option_text:String(r["型号款式"]??""),product_url:String(r["商品链接"]??""),quantity:q,unit_price:unitPrice,line_total:unitPrice*q});
  }
  return orders;
}
