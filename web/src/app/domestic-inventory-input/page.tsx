      ...
      <div style={topBarStyle}>
        <div>
          ...
          <p style={subTextStyle}>
            모바일은 텍스트 붙여넣기, PC는 HTML 업로드를 사용할 수 있어.
          </p>
        </div>
...
function extractImageInfosFromHtml(doc: Document) {
  ...
  return imgs
    .map((img) => {
      ...
      const qtyText = findNearbyQuantityText(img);
    ...
    .filter((info) => {
      ...
      if (
        info.imageUrl.includes("down-arrow") ||
        info.imageUrl.endsWith(".svg") ||
        info.imageUrl.includes("sprite") ||
        info.imageUrl.includes("nav-") ||
        info.imageUrl.includes("logo") ||
        info.imageUrl.includes("timeline") ||
        info.imageUrl.includes("transparent-pixel")
      ) {
        return false;
      }
      ...
      return /buy it again|view your item|return items|track package|problem with order/i.test(text);
    });
}
...
function getNearbyText(el: Element) {
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    if (/buy it again|view your item|track package|return items|problem with order/i.test(text)) {
      return text;
    }
