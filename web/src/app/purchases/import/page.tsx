...
export default function PurchaseImport(){
 ...
 const runText=async(action:string)=>{if(action==="preview"&&!text.trim())return setMsg("GPT가 정리한 텍스트를 붙여넣어줘.");if(action==="save"&&!preview?.purchase)return setMsg("먼저 미리보기를 눌러 내용을 확인해줘.");setBusy(true);setMsg(action==="preview"?"분석 중...":"매입관리에 등록 중...");try{const payload=action==="save"?{mode:action,purchase:preview.purchase}:{mode:action,text};const res=await fetch("/api/purchases/import",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),j=await res.json();if(!res.ok)throw new Error(j.message||"실패");if(action==="preview"){setPreview({purchase:j.purchase});setMsg(`${j.purchase.items.length}개 상품을 확인했어. 아래 항목을 수정한 뒤 등록하면 돼.`)}else{setMsg(`${j.itemCount}개 상품 매입관리 등록 완료`);setPreview(null)}}catch(e){setMsg(e instanceof Error?e.message:"실패")}finally{setBusy(false)}};
...
function PurchasePreview({purchase,onChange}:{purchase:any;onChange:(purchase:any)=>void}){const editHeader=(key:string,value:string)=>onChange({...purchase,[key]:key==="local_shipping"?Number(value):value});const editItem=(index:number,key:string,value:string)=>onChange({...purchase,items:purchase.items.map((item:any,i:number)=>i!==index?item:{...item,[key]:["unit_price","quantity","component_count"].includes(key)?(value===""?null:Number(value)):value})});return <section style={previewBox}><h2 style={h2}>미리보기 수정</h2><div style={editGrid}><Edit label="거래처명" value={purchase.supplier} onChange={v=>editHeader("supplier",v)}/><Edit label="매입일" type="date" value={purchase.purchased_at} onChange={v=>editHeader("purchased_at",v)}/><Edit label="명세서/주문번호" value={purchase.invoice_number} onChange={v=>editHeader("invoice_number",v)}/><Edit label="국내 배송비" type="number" value={String(purchase.local_shipping||0)} onChange={v=>editHeader("local_shipping",v)}/><Edit label="전체 메모" value={purchase.memo||""} onChange={v=>editHeader("memo",v)}/></div>{purchase.items.map((item:any,index:number)=><article key={index} style={editCard}><div style={thumbColumn}>{item.image_url?<img src={item.image_url} alt="대표 이미지" style={thumb}/>:<div style={noImage}>IMAGE</div>}</div><div style={itemEditor}><div style={itemTitle}>상품 #{index+1}</div><div style={editGrid}><Edit label="상품명" value={item.product_name} onChange={v=>editItem(index,"product_name",v)}/><SelectEdit label="작품명" value={item.series_name} options={seriesOptions} onChange={v=>editItem(index,"series_name",v)}/><SelectEdit label="타입" value={item.item_type} options={typeOptions} onChange={v=>editItem(index,"item_type",v)}/><Edit label="옵션/종류" value={item.option_text||""} onChange={v=>editItem(index,"option_text",v)}/><Edit label="매입 단가(원)" type="number" value={String(item.unit_price??"")} onChange={v=>editItem(index,"unit_price",v)}/><Edit label="매입 수량" type="number" value={String(item.quantity??"")} onChange={v=>editItem(index,"quantity",v)}/><Edit label="박스당 구성품" type="number" value={String(item.component_count??"")} onChange={v=>editItem(index,"component_count",v)}/><Edit label="대표 이미지 URL" value={item.image_url||""} onChange={v=>editItem(index,"image_url",v)}/><Edit label="라인업 이미지 URL" value={item.lineup_image_url||""} onChange={v=>editItem(index,"lineup_image_url",v)}/><Edit label="상품 URL" value={item.product_url||""} onChange={v=>editItem(index,"product_url",v)}/><Edit label="메모" value={item.memo||""} onChange={v=>editItem(index,"memo",v)}/></div></div></article>)}</section>}
