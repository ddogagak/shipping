...
const JAPAN_PROMPT=`아래 일본 구매대행/배송대행 신청내역 페이지 소스(HTML/마크다운/텍스트)를 확인해서 매입관리 등록용 텍스트로 정리해줘.

[규칙]
- 반드시 아래 양식만 출력하고 설명은 쓰지 않는다.
- 구매완료 상품만 정리한다.
- 신청서번호가 같으면 하나의 PURCHASE로 통합한다. 신청서번호가 다르면 PURCHASE를 나눈다.
- SUPPLIER에는 실제 원본 판매처를 적는다. 여러 판매처가 한 신청서에 섞여 있으면 "일본 구매대행"으로 적고 각 ITEM MEMO에 판매처를 적는다.
- PURCHASE_DATE는 페이지에 표시된 구매일을 쓴다.
- INVOICE_NO는 신청서번호를 쓴다.
- LOCAL_SHIPPING은 일본 내 배송비가 명확하게 확인될 때만 숫자로 적고, 확인되지 않으면 0으로 둔다.
- UNIT_PRICE는 페이지에 표시된 해당 상품의 엔화 가격 숫자를 그대로 쓴다. 임의로 원화 환산하지 않는다.
- QTY는 페이지에 표시된 수량을 쓴다.
- 상품명이 영어로 단순화되어 있어도 자료에 보이는 원본 상품명을 우선 사용하고, 더 구체적인 상품명이 확인되지 않으면 보이는 상품명을 그대로 쓴다.
- 작품명은 헌터헌터/귀멸의칼날/나의히어로아카데미아/프리렌/진격의거인/치이카와/나루토/기타 중 하나로 쓴다.
- TYPE은 아크릴/지류/뱃지/피규어/키링/기타 중 하나로 쓴다.
- 상품 링크 href를 PRODUCT_URL에 반드시 넣는다. 이미지 src가 확인되면 IMAGE에 반드시 넣는다.
- BOX_COUNT는 한 박스 안 판매 단위 수가 명확할 때만 적고, 단품이면 1, 알 수 없으면 빈칸으로 둔다.
- 아이템번호, 판매처, 운송장 등 추적에 유용한 정보는 ITEM MEMO에 적는다.
- 같은 상품이라도 URL/가격/옵션이 다르면 임의로 합치지 않는다.

[출력 양식]
=== PURCHASE ===
SUPPLIER: 판매처 또는 일본 구매대행
PURCHASE_DATE: YYYY-MM-DD
INVOICE_NO: 신청서번호
LOCAL_SHIPPING: 일본 내 배송비
MEMO: 일본 구매대행
