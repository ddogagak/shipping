...
function PurchasePreview({purchase,onChange}:{purchase:any;onChange:(purchase:any)=>void}){
 ...
 return <section style={previewBox}><h2 style={h2}>미리보기 수정</h2><p style={pasteGuide}>BAND/웹에서 이미지를 복사한 뒤 원하는 상품의 이미지 칸을 클릭하고 <b>Ctrl+V</b> 하면 대표 이미지로 들어가.</p>{pasteMsg&&<div style={pasteNotice}>{pasteMsg}</div>}<div style={editGrid}><Edit label="거래처명" value={purchase.supplier} onChange={v=>editHeader("supplier",v)}/><Edit label="매입일" type="date" value={purchase.purchased_at} onChange={v=>editHeader("purchased_at",v)}/><Edit label="명세서/주문번호" value={purchase.invoice_number} onChange={v=>editHeader("invoice_number",v)}/><Edit label="국내 배송비" type="number" value={String(purchase.local_shipping||0)} onChange={v=>editHeader("local_shipping",v)}/><Edit label="전체 메모" value={purchase.memo||""} onChange={v=>editHeader("memo",v)}/></div>{purchase.items.map((item:any,index:number)=><article key={index} style={editCard}><div style={thumbColumn}><div role="button" tabIndex={0} title="클릭 후 Ctrl+V로 이미지 붙여넣기" onPaste={e=>void pasteImage(index,e)} style={{...pasteTarget,...(pastingIndex===index?pasteTargetBusy:{})}}>{pastingIndex===index?<div style={noImage}>업로드중</div>:item.image_url?<img src={item.image_url} alt="대표 이미지" style={thumb}/>:<div style={noImage}>클릭<br/>Ctrl+V</div>}<span style={pasteHint}>{item.image_url?"다시 붙여넣기":"이미지 붙여넣기"}</span></div></div><div style={itemEditor}><div style={itemTitle}>상품 #{index+1}</div><div style={editGrid}><Edit label="상품명" value={item.product_name} onChange={v=>editItem(index,"product_name",v)}/><SelectEdit label="작품명" value={item.series_name} options={seriesOptions} onChange={v=>editItem(index,"series_name",v)}/><SelectEdit label="타입" value={item.item_type} options={typeOptions} onChange={v=>editItem(index,"item_type",v)}/><Edit label="옵션/종류" value={item.option_text||""} onChange={v=>editItem(index,"option_text",v)}/><Edit label="매입 단가(원)" type="number" value={String(item.unit_price??"")} onChange={v=>editItem(index,"unit_price",v)}/><Edit label="매입 수량" type="number" value={String(item.quantity??"")} onChange={v=>editItem(index,"quantity",v)}/><Edit label="박스당 구성품" type="number" value={String(item.component_count??"")} onChange={v=>editItem(index,"component_count",v)}/><Edit label="대표 이미지 URL" value={item.image_url||""} onChange={v=>editItem(index,"image_url",v)}/><Edit label="라인업 이미지 URL" value={item.lineup_image_url||""} onChange={v=>editItem(index,"lineup_image_url",v)}/><Edit label="상품 URL" value={item.product_url||""} onChange={v=>editItem(index,"product_url",v)}/><Edit label="메모" value={item.memo||""} onChange={v=>editItem(index,"memo",v)}/></div></div></article>)}</section>
}
