      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={updateTracking} disabled={updating || !selectedCount} style={darkButtonStyle}>
            {updating ? "업데이트 중..." : `선택 ${selectedCount}건 DB 업데이트`}
          </button>
        </div>
      ...
      {rows.length ? (
        <section style={{ ...cardStyle, marginTop: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
            <h2 style={{ marginTop: 0 }}>미리보기 결과</h2>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={`${row.row_index}-${index}`} style={{ background: row.selected ? "#f8fafc" : "#fff" }}>
                    ...
                    <td style={tdStyle}>{row.order_suffixes.join(", ")}</td>
                    ...
                    <td style={tdStyle}>{row.local_tracking_number || "-"}</td>
