...
function shipping(row: DomesticOrder) {
  if (Array.isArray(row.domestic_shipping)) {
    return row.domestic_shipping[0] || null;
  }
  return row.domestic_shipping || null;
}
...
function toExcelRow(row: DomesticOrder) {
  return {
    ...
    주문건수: String(row.order_count || 1),
    최초주문일: row.first_order_date || "",
    아이템: Array.isArray(row.source_order_dates)
      ? row.source_order_dates.join(" / ")
      : "",
    상품금액합계: formatWon(row.item_total_price),
  };
}
...
export default function DomesticOrdersPage() {
  ...
  const filteredRows = useMemo(() => {
    return rows
      .filter((row) => {
        ...
        if (platforms.length && !platforms.includes(row.platform)) return false;
        if (orderStatuses.length && !orderStatuses.includes(row.order_status || "accepted")) {
          return false;
        }
        if (shippingStatuses.length && !shippingStatuses.includes(shippingStatus)) {
          return false;
        }
        if (shippingTypes.length && !shippingTypes.includes(shippingType)) return false;
      ...
      .sort((a, b) => {
        const sA = shipping(a)?.shipping_status || "start";
        const sB = shipping(b)?.shipping_status || "start";
        const shipOrder = ["start", "excel_exported", "uploaded", "done"];
        const shipDiff = shipOrder.indexOf(sA) - shipOrder.indexOf(sB);
        if (shipDiff !== 0) return shipDiff;

        const orderOrder = ["accepted", "checked", "done"];
        const orderDiff =
          orderOrder.indexOf(a.order_status || "accepted") -
          orderOrder.indexOf(b.order_status || "accepted");
        if (orderDiff !== 0) return orderDiff;

        return String(a.first_order_date || "").localeCompare(
          String(b.first_order_date || "")
        );
      });
  }, [rows, platforms, orderStatuses, shippingStatuses, shippingTypes, q]);
  ...
  function updateSelected(orderId: string, selected: boolean) {
    setRows((prev) =>
      prev.map((row) =>
        row.order_id === orderId ? { ...row, selected } : row
      )
    );
  }
      ...
      <section style={{ ...cardStyle, marginTop: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <input
            ...
            placeholder="고객주문번호, 닉네임, 수취인, 전화번호, 주소, 운송장 검색"
      ...
      <section style={{ ...cardStyle, marginTop: 16 }}>
        <div style={actionBarStyle}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 800 }}>
            <input
              ...
              checked={
                ...
                filteredRows.every((row) => row.selected)
              }
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button type="button" onClick={() => patch("order_done")} style={purpleButtonStyle}>
              주문완료 처리
            </button>
                ...
                {filteredRows.map((row) => {
                  ...
                  return (
                    <tr key={row.order_id}>
                      ...
                      <td style={{ ...tdStyle, minWidth: 320 }}>{row.address || ""}</td>
                      <td style={tdStyle}>{row.order_count || 1}</td>
                      <td style={tdStyle}>{row.first_order_date || ""}</td>
                      <td style={tdStyle}>{formatWon(row.item_total_price)}</td>
                      <td style={tdStyle}>
                        {label(ORDER_STATUS_OPTIONS, row.order_status || "accepted")}
                      </td>
                      <td style={tdStyle}>
                        {label(SHIPPING_STATUS_OPTIONS, s?.shipping_status || "start")}
                      </td>
                      <td style={tdStyle}>{s?.shipping_type || "일반택배"}</td>
                      <td style={tdStyle}>{s?.tracking_number || ""}</td>
                    </tr>
