...
export async function POST(req: Request) {
  try {
    ...
    if (!recipientName) {
      return NextResponse.json({ ok: false, message: "수취인명을 입력해주세요." }, { status: 400 });
    }
    ...
    if (!postalCode) {
      return NextResponse.json({ ok: false, message: "우편번호를 입력해주세요." }, { status: 400 });
    }
    ...
    if (!itemSummary) {
      return NextResponse.json({ ok: false, message: "신청 물품을 입력해주세요." }, { status: 400 });
    }
    ...
    const { error: orderError } = await supabase.from("domestic_order").insert({
      order_id: orderId,
      platform: "Kuji",
    ...
    const { error: itemError } = await supabase.from("domestic_order_item").insert({
      order_id: orderId,
    ...
    const { error: shippingError } = await supabase.from("domestic_shipping").insert({
      order_id: orderId,
      carrier: "",
      tracking_number: "",
    ...
    if (shippingError) {
      return NextResponse.json({ ok: false, message: shippingError.message }, { status: 500 });
    }
    ...
    return NextResponse.json(
      {
        ok: false,
        message: error instanceof Error ? error.message : "신청 저장 실패",
      },
      { status: 500 }
    );
  }
