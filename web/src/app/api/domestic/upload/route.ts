...
function duplicateKey(customerOrderNo: string, firstOrderDate: string) {
  return `${safeText(customerOrderNo)}|${safeText(firstOrderDate)}`;
}
...
function normalizeWiseDate(value: string) {
  const text = safeText(value);
  const match = text.match(/(\d{4})\.\s*(\d{2})\.\s*(\d{2})\s*\/\s*(\d{2}):(\d{2})/);
  if (!match) return "";
  return `${match[1]}.${match[2]}.${match[3]} ${match[4]}:${match[5]}`;
}

function extractWiseOrderDate(block: string) {
  const match = block.match(/\d{4}\.\s*\d{2}\.\s*\d{2}\s*\/\s*\d{2}:\d{2}/);
  return match ? normalizeWiseDate(match[0]) : "";
}

function splitWiseBlocks(text: string) {
...
function parseWiseText(text: string, platform: Platform): ParsedRow[] {
  const blocks = splitWiseBlocks(text);
  ...
  blocks.forEach((block) => {
    ...
    const orderDate = extractWiseOrderDate(block);
    ...
    const parsedItems = parseWiseItems(block);
...
function parseDomesticText(text: string, platform: Platform): ParsedRow[] {
  ...
  return parseWiseText(text, platform);
}
...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const customerOrderNos = parsed.map((row) => row.customerOrderNo).filter(Boolean);

      const { data: existing, error } = customerOrderNos.length
        ? await supabase
            .from("domestic_order")
            .select("order_id, customer_order_no, first_order_date")
            .in("customer_order_no", customerOrderNos)
        : { data: [], error: null };
      ...
      const existingSet = new Set(
        (existing || []).map((row: any) =>
          duplicateKey(row.customer_order_no || row.order_id, row.first_order_date || "")
        )
      );

      const fresh = parsed.filter(
        (row) => !existingSet.has(duplicateKey(row.customerOrderNo, row.firstOrderDate))
      );

      const duplicated = parsed
        .filter((row) =>
          existingSet.has(duplicateKey(row.customerOrderNo, row.firstOrderDate))
        )
        .map((row) => ({
          ...row,
          selected: false,
          disabled: true,
          reason: "같은 고객주문번호 + 같은 최초주문일 중복",
        }));
    ...
    const customerOrderNos = selectedRows.map((row: ParsedRow) => row.customerOrderNo);
    ...
    const orderPayload = freshRows.map((row: ParsedRow) => ({
      order_id: makeDomesticOrderId(row),
      customer_order_no: row.customerOrderNo,
